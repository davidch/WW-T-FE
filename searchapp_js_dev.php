...
$documentdata = array(
  ...
  'meta' => array(
    'last_modified'=>'Mon, 04 Mar 2013 10:55:55 CST',  
    'last_modified_by_username' => 'tzvi.eleff',  
    'last_modified_by_fullname' => 'tzvi.eleff',
    'publish_date' => '',
    'expiry_date' => '',
    'content_version' => '6.0',
...
function convert($time, $timezone = '', $format = 'Y-m-d H:i:s'){
  ...
  if($datetime = new DateTime($time)){
      if(empty($timezone)){ # If empty timezone, fetch server timezone
        $timezone = date('e');
      }
...
$utc_str = gmdate("M d Y H:i:s T", time());
$utc = strtotime($utc_str);

echo "Current gmdate generated time UTC: " . $utc . "<br/>";
echo "Current gmdate generated time UTC CONVERTED: " . convert($utc, 'EDT') . "<br/>";
echo "Current date generated time UTC COMPARISON: " . date("Y-m-d H:i:s T", $utc) . "<br/><br/>";

//fetch indexed document
$type = $elasticaClient->getIndex('www')->getType('page');
$doc = $type->getDocument(1571); 
//print_r($doc);

//$stamp = strtotime($documentdata['meta']['last_modified']);
//$UTC = new DateTimeZone("UTC");

$stamp = strtotime('Tue, 19 Mar 2013 11:02:55 CST');

echo "Current T4 time CST: " . $stamp . "<br/>";
echo "Current T4 time CST CONVERTED: " . convert($stamp, 'CST') . "<br/>";
echo "Current T4 time CST COMPARISON: " . date("Y-m-d H:i:s T", $stamp) . "<br/>";



$thattime = gmdate("M d Y H:i:s T", $stamp);
